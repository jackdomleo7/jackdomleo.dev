      ...
      <ul>
        <li v-for="(work, index) in workHistory" :key="index" :aria-setsize="workHistory.length" :aria-posinset="index + 1">
          ...
          <div class="work__time">
            ...
            <time :datetime="getDate(work.start).datetime">{{ getDate(work.start).date }}</time> <span aria-label="to">-</span> <time :datetime="work.end === 'present' ? getDate(new Date()).datetime : getDate(work.end).datetime">{{ work.end === 'present' ? 'present' : getDate(work.end).date }} <span v-if="work.end === 'present'" class="sr-only">day</span></time>
          </div>
  ...
  private readonly workHistory: IWork[] = [
    {
      ...
      company: 'Mitrefinch',
      title: 'Developer',
      description: 'Creating multitennant SaaS applications such as payroll and access control using Vue.js and TypeScript'
    },
    {
      ...
      description: 'Created a centralised repository of consistent styles using SCSS, BEM and PostCSS to consume in new products'
    },
    {
      ...
      description: 'Used the Grav CMS to maintain the airline\'s website and React.js to maintain the airline\'s intranet'
    },
    {
      ...
      description: 'Worked part-time for this fast-paced Nottingham high street store during my time in Sixth Form trying to balance my work hours, study hours and personal life'
    },
    {
